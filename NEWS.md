...
- Fixed some typos in `single-timeseries-rt-estimation.Rmd`. The `WORDLIST` used by spelling has also been updated to eliminate false positives. Future typos will now generate an error in the continuous integration check. This will help us to catch them as early as possible. See #341 by @Bisaloo and reviewed by @seabbs.
